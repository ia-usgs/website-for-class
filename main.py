make_ids = {
    ...
    'VinFast': 65,
    'Volvo': 66,
    # Additional makes if needed
}

vehicle_model_year_entries = [ (1909, 'Ford', 'Model T'),
...
# Placeholder for the SQL update commands
# Name of the file to save the SQL commands
filename = 'update_commands.txt'

with open(filename, 'w') as file:
    for year, make, model in vehicle_model_year_entries:
        if make in make_ids:
            make_id = make_ids[make]
            # Constructing the SQL query. Assuming your table name is `vehicles` and columns for make_id, model, and year are `make_id`, `model`, `year` respectively.
            # This also assumes that the combination of model and year is unique and can be used to identify a row in your table.
            sql_update = f"UPDATE models SET make_id = {make_id} WHERE name = '{model}';\n"
            file.write(sql_update)

print(f"SQL update commands have been saved to {filename}.")